...
function Subjects() {
  ...
  const openModal = (record = null) => {
    ...
    if (record) {
      form.setFieldsValue({
        ...
        teacherId: teachers.find(t => t.fullname === record.teacherName)?.id,
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_, record) => (
        <>
          <Button type="primary" size="small" onClick={() => openModal(record)}>Edit</Button>
          <Button
            danger
            size="small"
            style={{ marginLeft: 8 }}
    ...
    <Card title="Subjects" extra={
      ...
      >
        <Form layout="vertical" form={form}>
          ...
          <Form.Item label="Assign Teacher" name="teacherId" rules={[{ required: true }]}>

...
function Subjects() {
  ...
  const openModal = (subject = null) => {
    if (subject) {
      ...
      setFormData({
        ...
        department: subject.subjectDepartment || '' // ✅ Prefill department if exists
      });
    } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const errMsg = error.response?.data?.message || `Failed to ${msg} subject.`;
      toast.error(errMsg);
    }
  };
...
.filter(sub => {
  ...
  return (
    ...
    sub.subjectDepartment?.toLowerCase().includes(term) ||
            ...
            <tbody>
              {displayedSubjects.map((sub) => (
                <tr key={sub.id}>
                  ...
                  <td>{sub.credits}</td>
                  <td>{sub.department || "—"}</td> {/* ✅ Display department */}
                  <td>{sub.teacherName}</td>
                  <td>{sub.subjectDepartment}</td>
                  <td>

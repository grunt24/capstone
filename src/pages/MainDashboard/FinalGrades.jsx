// src/components/FinalsGradesTable.js
...
const MANUAL_INSERT_API = "/GradeCalculation/manual-insert-finals";
const ALL_STUDENTS_API = "/Auth/all-students";

function FinalsGradesTableContent() {
  ...
  const handleUpload = async ({ file }) => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        toast.error(
          response.data?.message || "Failed to upload file with a non-200 response."
        );
      }
    } catch (err) {
      console.error(err);
  ...
  const columns = [
    ...
    {
      title: "Rounded Grade",
      dataIndex: "totalFinalsGradeRounded",
      key: "totalFinalsGradeRounded",
    },
    {
      title: "Equivalent",
  ...
  const chartData = [
    ...
    {
      name: "Finals Exam",
      score: selectedStudent?.finalsScore,
      total: selectedStudent?.finalsTotal,
    },
  ];
    ...
    <div>
      ...
      >
        {selectedStudent && (
          <>
            ...
            >
              <span>Name: </span>
              {selectedStudent.studentFullName} - Finals Breakdown
            </h3>
            <div
              style={{
                fontWeight: "bold",
            ...
            >
              ...
              {selectedStudent.subjectTeacher}
            </div>
            <Descriptions bordered size="small" column={2}>
              ...
              <Descriptions.Item label="Finals Exam">
                {selectedStudent.finalsScore} / {selectedStudent.finalsTotal}
              </Descriptions.Item>
              <Descriptions.Item label="Final Grade">
                {selectedStudent.totalFinalsGrade >= 75 ? (
                  <Tag color="green">
                    {selectedStudent.totalFinalsGrade} (Pass)
                  </Tag>
                ) : (
                  <Tag color="red">
                    {selectedStudent.totalFinalsGrade} (Fail)
                  </Tag>
                )}
              </Descriptions.Item>
      ...
      >
        ...
        >
          ...
          <Form.Item label="Finals Score" name="finalsScore">
            <Input type="number" />
          </Form.Item>
          <Form.Item label="Finals Total" name="finalsTotal">
            <Input type="number" />
          </Form.Item>
...
function FinalsGradesTable() {
  return <FinalsGradesTableContent />;
}

export default FinalsGradesTable;
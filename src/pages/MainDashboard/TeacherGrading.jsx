...
function TeacherGrading() {
    ...
    useEffect(() => {
        ...
        if (userDetails && userDetails.id) {
            const fetchSubjects = async () => {
                setLoading(true);
                try {
                    const res = await axiosInstance.get(`/Subjects/teacher/${userDetails.id}`);
    ...
    useEffect(() => {
        if (selectedStudent && selectedSubject) {
            ...
            const fullStudent = selectedSubject.students.find(s => s.studentId === selectedStudent.studentId);
            if (fullStudent) {
        ...
        <div className="container mt-5">
            <div className="card shadow-sm">
                ...
                <div className="card-body">
                    {/* Subject Selection */}
                    <div className="mb-4">
                        <label htmlFor="subject-select" className="form-label">Select a Subject</label>
                        <select
                            id="subject-select"
                    ...
                    {selectedSubject && (
                        <div className="mb-4">
                            <label htmlFor="student-select" className="form-label">Select a Student</label>
                            <select
                                id="student-select"
                                className="form-select"
                                value={selectedStudent?.studentId || ''}
                                onChange={(e) => {
                                    const student = selectedSubject.students.find(s => s.studentId === parseInt(e.target.value));
                    ...
                    {selectedStudent && (
                        <>
                                            ...
                                            <tr key={index}>
                                                <td className="align-middle">
                                                    <input
                                                        ...
                                                        onChange={(e) => handleScoreChange(index, 'scoreType', e.target.value)}
                                                    />
                                                </td>
                                                <td className="align-middle">
                                                    <input
                                                ...
                                                <td className="align-middle">
                                                    <input
                                                        ...
                                                        placeholder="Total"
                                                        value={score.total}
                                                        onChange={(e) => handleScoreChange(index, 'total', e.target.value)}
                                                    />
                                                </td>
                                                <td className="align-middle text-center">
                                                    <button
                                                        ...
                                                        onClick={() => {
                                                            const newScores = studentData.scores.filter((_, i) => i !== index);
                                                            setStudentData(prev => ({ ...prev, scores: newScores }));
                                                        }}
                                                    >
                                                        Delete
                                                    </button>
                            ...
                            <button className="btn btn-secondary btn-sm mb-3" onClick={handleAddScore}>
                                + Add Score
                            </button>

                            <div className="d-flex justify-content-between align-items-center mt-3">
                                <h5 className="mb-0">Calculated Percentage: {calculatePercentage()}%</h5>
